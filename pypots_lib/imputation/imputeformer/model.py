...
from ...nn.modules.loss import Criterion, MAE, MSE, ImputeFormerLoss
...
class ImputeFormer(BaseNNImputer):
    ...
    f1_loss_weight :
        The weight for the frequency regularization loss (default: 0.01).

    base_loss :
        The base loss function to use for ORT and MIT (default: MAE).
    ...
    def __init__(
        ...
        base_loss: Union[Criterion, type] = MAE,
        batch_size: int = 32,
        epochs: int = 100,
        patience: Optional[int] = None,
        training_loss: Union[Criterion, type] = ImputeFormerLoss,
        ...
        # If using ImputeFormerLoss, create instance with weights and base loss
        if training_loss == ImputeFormerLoss:
            loss_func = base_loss() if base_loss else MAE()
            self.training_loss = ImputeFormerLoss(MIT_weight=MIT_weight, ORT_weight=ORT_weight, 
                                                  f1_loss_weight=f1_loss_weight, loss_func=loss_func)
    ...
    def fit(
        ...
        train_dataset = DatasetForSAITS(train_set, return_X_ori=False, return_y=False, file_type=file_type, rate=self.mit_rate)
        ...
        if val_set is not None:
            ...
            val_dataset = DatasetForSAITS(val_set, return_X_ori=True, return_y=False, file_type=file_type, rate=self.mit_rate)
